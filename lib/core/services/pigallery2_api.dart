...
class PiGallery2API {
  ...
  Future<ApiResponse<Directory>> getDirectories({
    ...
    try {
      http.Response response = await client.get(uri, headers: getHeaders(sessionData));
      Map<String, dynamic> result = json.decode(response.body);
      if (result["error"] == null) {
        return ApiResponse(code: response.statusCode, result: Directory.fromJson(result['result']['directory']));
      } else {
